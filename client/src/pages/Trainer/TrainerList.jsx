...
import { getAllTrainers_s, deleteTrainer } from '../../api/simpleFormsApi';
...
export default function TrainerList() {
  ...
  const fetchData = async () => {
    ...
    try { const res = await getAllTrainers_s(); setRows(res.data.data || []); }
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(`Delete trainer #${id}?`)) return;
    try { await deleteTrainer(id); setRows((prev) => prev.filter((r) => r.id !== id)); }
  ...
  const filtered = useMemo(() => {
    ...
    data.sort((a, b) => {
      ...
      if (sortKey === 'commission_rate') { va = parseFloat(va); vb = parseFloat(vb); }
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => navigate('/trainers/new')}>+ New Trainer</button>
      </div>

      <div className="card">
        <div className="card-body" style={{ paddingBottom: 0 }}>
          <div className="filter-bar">
            <input className="form-input" style={{ minWidth: 220 }} placeholder="Search ID, name, specialization, phone..." value={search} onChange={(e) => setSearch(e.target.value)} />
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th className="sortable" onClick={() => handleSort('trainer_name')}>Name {sortIcon('trainer_name')}</th>
                    <th className="sortable" onClick={() => handleSort('specialization')}>Specialization {sortIcon('specialization')}</th>
                    ...
                    <th className="sortable" style={{ textAlign: 'right' }} onClick={() => handleSort('commission_rate')}>Commission Rate {sortIcon('commission_rate')}</th>
                    <th style={{ textAlign: 'center' }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {paginated.length === 0
                    ? <tr><td colSpan={7}><div className="state-box">No trainers found.</div></td></tr>
                    : paginated.map((r) => (
                        <tr key={r.id}>
                          <td>
                            <span className="badge badge-green" style={{ cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: 3 }} onClick={() => navigate(`/trainers/${r.id}`)}>
                              {r.id}
                            </span>
                          </td>
                          <td style={{ fontWeight: 500 }}>{r.trainer_name}</td>
                          <td>{r.specialization || '—'}</td>
                          <td>{r.phone || '—'}</td>
                          <td>{r.email || '—'}</td>
                          <td style={{ textAlign: 'right' }}>{fmt(r.commission_rate)}%</td>
                          <td style={{ textAlign: 'center' }}>
                            <button className="action-btn edit" onClick={() => navigate(`/trainers/${r.id}/edit`)}>Edit</button>
