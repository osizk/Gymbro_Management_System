...
export default function EquipmentItemList() {
  ...
  const fetchData = async () => {
    ...
    try { const res = await getAllEquipmentItems(); setRows(res.data.data || []); }
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(`Delete equipment #${id}?`)) return;
    try { await deleteEquipmentItem(id); setRows((prev) => prev.filter((r) => r.id !== id)); }
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => navigate('/equipment-items/new')}>+ New Equipment</button>
      </div>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th className="sortable" onClick={() => handleSort('equipment_name')}>Equipment Name {sortIcon('equipment_name')}</th>
                    <th className="sortable" onClick={() => handleSort('category_name')}>Category {sortIcon('category_name')}</th>
                    <th className="sortable" onClick={() => handleSort('purchase_date')}>Purchase Date {sortIcon('purchase_date')}</th>
                ...
                <tbody>
                  {paginated.length === 0
                    ? <tr><td colSpan={6}><div className="state-box">No equipment found.</div></td></tr>
                    : paginated.map((r) => {
                        const sc = STATUS_COLORS[r.status] || STATUS_COLORS.ACTIVE;
                        return (
                          <tr key={r.id}>
                            <td>
                              <span className="badge badge-green" style={{ cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: 3 }} onClick={() => navigate(`/equipment-items/${r.id}`)}>
                                {r.id}
                              </span>
                            </td>
                            ...
                            <td style={{ textAlign: 'center' }}>
                              <button className="action-btn edit" onClick={() => navigate(`/equipment-items/${r.id}/edit`)}>Edit</button>
