...
import { getExpenseCategoryById } from '../../api/simpleFormsApi';
...
export default function ExpenseCategoryView() {
  ...
  useEffect(() => {
    getExpenseCategoryById(id).then((r) => setData(r.data.data)).catch(() => setError('Failed to load.')).finally(() => setLoading(false));
  }, [id]);
  ...
  return (
    <div>
      <div className="page-header">
        <div><h1 className="page-title">Expense Category</h1><p className="page-subtitle">#{data.id}</p></div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button className="btn btn-secondary" onClick={() => navigate('/expense-categories')}>← Back</button>
          <button className="btn btn-secondary" onClick={() => navigate(`/expense-categories/${id}/edit`)}>Edit</button>
        </div>
      </div>
