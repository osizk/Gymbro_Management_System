const svc = require('../services/expenseCategoryService');
...
exports.getById = handle(async (req, res) => {
  ...
  if (!d) return res.status(404).json({ success: false, message: 'Expense category not found' });
...
exports.remove  = handle(async (req, res) => {
  ...
  if (!ok) return res.status(404).json({ success: false, message: 'Expense category not found' });
