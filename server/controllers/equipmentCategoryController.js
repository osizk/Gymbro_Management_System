const svc = require('../services/equipmentCategoryService');
...
exports.getById = handle(async (req, res) => {
  ...
  if (!d) return res.status(404).json({ success: false, message: 'Equipment category not found' });
  res.json({ success: true, data: d });
});
exports.create  = handle(async (req, res) => {
  if (!req.body.category_name) return res.status(400).json({ success: false, message: 'category_name is required' });
...
exports.remove  = handle(async (req, res) => {
  ...
  if (!ok) return res.status(404).json({ success: false, message: 'Equipment category not found' });
