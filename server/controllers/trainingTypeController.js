const svc = require('../services/trainingTypeService');
...
exports.getById = handle(async (req, res) => {
  ...
  if (!d) return res.status(404).json({ success: false, message: 'Training type not found' });
  res.json({ success: true, data: d });
});
exports.create  = handle(async (req, res) => {
  const { type_name, default_hourly_rate } = req.body;
  if (!type_name)           return res.status(400).json({ success: false, message: 'type_name is required' });
  if (!default_hourly_rate) return res.status(400).json({ success: false, message: 'default_hourly_rate is required' });
...
exports.remove  = handle(async (req, res) => {
  ...
  if (!ok) return res.status(404).json({ success: false, message: 'Training type not found' });
