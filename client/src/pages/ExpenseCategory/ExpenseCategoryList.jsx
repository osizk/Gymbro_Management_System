...
import { getAllExpenseCategories_s, deleteExpenseCategory } from '../../api/simpleFormsApi';
...
export default function ExpenseCategoryList() {
  ...
  const fetchData = async () => {
    ...
    try { const res = await getAllExpenseCategories_s(); setRows(res.data.data || []); }
    catch { setError('Failed to load expense categories.'); }
    finally { setLoading(false); }
  };
  useEffect(() => { fetchData(); }, []);

  const handleDelete = async (id) => {
    if (!window.confirm(`Delete expense category #${id}?`)) return;
    try { await deleteExpenseCategory(id); setRows((prev) => prev.filter((r) => r.id !== id)); }
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => navigate('/expense-categories/new')}>+ New Category</button>
      </div>
          ...
          <div className="table-wrapper">
            ...
            : (
              <table>
                ...
                <tbody>
                  {paginated.length === 0
                    ? <tr><td colSpan={3}><div className="state-box">No expense categories found.</div></td></tr>
                    : paginated.map((r) => (
                        <tr key={r.id}>
                          <td>
                            <span className="badge badge-green" style={{ cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: 3 }} onClick={() => navigate(`/expense-categories/${r.id}`)}>
                              {r.id}
                            </span>
                          </td>
                          <td style={{ fontWeight: 500 }}>{r.category_name}</td>
                          <td style={{ textAlign: 'center' }}>
                            <button className="action-btn edit" onClick={() => navigate(`/expense-categories/${r.id}/edit`)}>Edit</button>
