...
export default function ClassBookingList() {
  ...
  const fetchData = async () => {
    ...
    try { const res = await getAllClassBookings(); setRows(res.data.data || []); }
    catch { setError('Failed to load class bookings.'); }
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteClassBooking(id); setRows((prev) => prev.filter((r) => r.id !== id)); }
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => navigate('/class-bookings/new')}>+ New Booking</button>
      </div>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th className="sortable" onClick={() => handleSort('class_name')}>Class {sortIcon('class_name')}</th>
                    <th className="sortable" onClick={() => handleSort('member_name')}>Member {sortIcon('member_name')}</th>
                    <th className="sortable" onClick={() => handleSort('booking_date')}>Booking Date {sortIcon('booking_date')}</th>
                ...
                <tbody>
                  {paginated.length === 0
                    ? <tr><td colSpan={7}><div className="state-box">No bookings found.</div></td></tr>
                    : paginated.map((r) => {
                        const sc = STATUS_COLORS[r.status] || STATUS_COLORS.CONFIRMED;
                        return (
                          <tr key={r.id}>
                            <td>
                              <span className="badge badge-green" style={{ cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: 3 }} onClick={() => navigate(`/class-bookings/${r.id}`)}>
                                {r.id}
                              </span>
                            </td>
                            <td>
                              <span style={{ color: 'var(--gray-400)', fontSize: 12, marginRight: 4 }}>#{r.class_id}</span>
                              {r.class_name}
                            </td>
                            <td>
                            ...
                            <td style={{ fontSize: 13, color: 'var(--gray-600)' }}>{r.check_in_time || '—'}</td>
                            <td style={{ textAlign: 'center' }}>
                              <button className="action-btn edit" onClick={() => navigate(`/class-bookings/${r.id}/edit`)}>Edit</button>
