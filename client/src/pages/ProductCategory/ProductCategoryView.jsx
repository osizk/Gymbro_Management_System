...
import { getProductCategoryById } from '../../api/simpleFormsApi';
...
export default function ProductCategoryView() {
  ...
  useEffect(() => {
    getProductCategoryById(id).then((r) => setData(r.data.data)).catch(() => setError('Failed to load.')).finally(() => setLoading(false));
  }, [id]);
  ...
  return (
    <div>
      <div className="page-header">
        <div><h1 className="page-title">Product Category</h1><p className="page-subtitle">#{data.id}</p></div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button className="btn btn-secondary" onClick={() => navigate('/product-categories')}>← Back</button>
          <button className="btn btn-secondary" onClick={() => navigate(`/product-categories/${id}/edit`)}>Edit</button>
        </div>
      </div>
