...
import { getAllProductCategories_s, deleteProductCategory } from '../../api/simpleFormsApi';
...
export default function ProductCategoryList() {
  ...
  const fetchData = async () => {
    ...
    try { const res = await getAllProductCategories_s(); setRows(res.data.data || []); }
    catch { setError('Failed to load product categories.'); }
    finally { setLoading(false); }
  };
  useEffect(() => { fetchData(); }, []);

  const handleDelete = async (id) => {
    if (!window.confirm(`Delete product category #${id}?`)) return;
    try { await deleteProductCategory(id); setRows((prev) => prev.filter((r) => r.id !== id)); }
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => navigate('/product-categories/new')}>+ New Category</button>
      </div>
          ...
          <div className="table-wrapper">
            ...
            : (
              <table>
                ...
                <tbody>
                  {paginated.length === 0
                    ? <tr><td colSpan={3}><div className="state-box">No product categories found.</div></td></tr>
                    : paginated.map((r) => (
                        <tr key={r.id}>
                          <td>
                            <span className="badge badge-green" style={{ cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: 3 }} onClick={() => navigate(`/product-categories/${r.id}`)}>
                              {r.id}
                            </span>
                          </td>
                          <td style={{ fontWeight: 500 }}>{r.category_name}</td>
                          <td style={{ textAlign: 'center' }}>
                            <button className="action-btn edit" onClick={() => navigate(`/product-categories/${r.id}/edit`)}>Edit</button>
