...
export default function EquipmentCategoryForm() {
  ...
  useEffect(() => {
    ...
    getEquipmentCategoryById(id)
      ...
      .catch(() => alert('Failed to load equipment category'))
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      if (isEdit) { await updateEquipmentCategory(id, { category_name: categoryName }); navigate(`/equipment-categories/${id}`); }
      else { const res = await createEquipmentCategory({ category_name: categoryName }); navigate(`/equipment-categories/${res.data.data.id}`); }
