...
export default function MemberList() {
  ...
  const fetchData = async () => {
    ...
    try { const res = await getAllMembers(); setRows(res.data.data || []); }
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteMember(id); setRows((prev) => prev.filter((r) => r.id !== id)); }
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => navigate('/members/new')}>+ New Member</button>
      </div>

      <div className="card">
        <div className="card-body" style={{ paddingBottom: 0 }}>
          <div className="filter-bar">
            <input className="form-input" style={{ minWidth: 220 }} placeholder="Search ID, name, phone, email..." value={search} onChange={(e) => setSearch(e.target.value)} />
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th className="sortable" onClick={() => handleSort('member_name')}>Name {sortIcon('member_name')}</th>
                    <th className="sortable" onClick={() => handleSort('gender')}>Gender {sortIcon('gender')}</th>
                    ...
                    <th className="sortable" onClick={() => handleSort('join_date')}>Join Date {sortIcon('join_date')}</th>
                  ...
                  {paginated.length === 0
                    ...
                    : paginated.map((r) => {
                        ...
                        return (
                          <tr key={r.id}>
                            <td>
                              <span className="badge badge-green" style={{ cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: 3 }} onClick={() => navigate(`/members/${r.id}`)}>
                                {r.id}
                              </span>
                            </td>
                            ...
                            <td style={{ textAlign: 'center' }}>
                              <button className="action-btn edit" onClick={() => navigate(`/members/${r.id}/edit`)}>Edit</button>
