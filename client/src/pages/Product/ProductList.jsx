...
export default function ProductList() {
  ...
  const fetchData = async () => {
    ...
    try { const res = await getAllProducts_s(); setRows(res.data.data || []); }
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteProduct(id); setRows((prev) => prev.filter((r) => r.id !== id)); }
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => navigate('/products/new')}>+ New Product</button>
      </div>

      <div className="card">
        <div className="card-body" style={{ paddingBottom: 0 }}>
          <div className="filter-bar">
            <input className="form-input" style={{ minWidth: 220 }} placeholder="Search ID, name, category..." value={search} onChange={(e) => setSearch(e.target.value)} />
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th className="sortable" onClick={() => handleSort('product_name')}>Product Name {sortIcon('product_name')}</th>
                    <th className="sortable" onClick={() => handleSort('category_name')}>Category {sortIcon('category_name')}</th>
                    <th className="sortable" style={{ textAlign: 'right' }} onClick={() => handleSort('cost_price')}>Cost (฿) {sortIcon('cost_price')}</th>
                    <th className="sortable" style={{ textAlign: 'right' }} onClick={() => handleSort('selling_price')}>Sell (฿) {sortIcon('selling_price')}</th>
                    <th className="sortable" style={{ textAlign: 'right' }} onClick={() => handleSort('stock_quantity')}>Stock {sortIcon('stock_quantity')}</th>
                ...
                <tbody>
                  {paginated.length === 0
                    ? <tr><td colSpan={8}><div className="state-box">No products found.</div></td></tr>
                    : paginated.map((r) => {
                        const sc = STATUS_COLORS[r.status] || STATUS_COLORS.INACTIVE;
                        return (
                          <tr key={r.id}>
                            <td>
                              <span className="badge badge-green" style={{ cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: 3 }} onClick={() => navigate(`/products/${r.id}`)}>
                                {r.id}
                              </span>
                            </td>
                            <td style={{ fontWeight: 500 }}>{r.product_name}</td>
                            <td>{r.category_name || '—'}</td>
                            <td style={{ textAlign: 'right' }}>{fmt(r.cost_price)}</td>
                            <td style={{ textAlign: 'right', fontWeight: 600 }}>{fmt(r.selling_price)}</td>
                            <td style={{ textAlign: 'right' }}>{r.stock_quantity}</td>
                            <td><span className="badge" style={{ background: sc.bg, color: sc.color }}>{r.status}</span></td>
                            <td style={{ textAlign: 'center' }}>
                              <button className="action-btn edit" onClick={() => navigate(`/products/${r.id}/edit`)}>Edit</button>
