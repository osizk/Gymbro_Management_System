...
export default function StaffList() {
  ...
  const fetchData = async () => {
    ...
    try { const res = await getAllStaff_s(); setRows(res.data.data || []); }
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteStaff(id); setRows((prev) => prev.filter((r) => r.id !== id)); }
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => navigate('/staff/new')}>+ New Staff</button>
      </div>

      <div className="card">
        <div className="card-body" style={{ paddingBottom: 0 }}>
          <div className="filter-bar">
            <input className="form-input" style={{ minWidth: 220 }} placeholder="Search ID, name, position, phone..." value={search} onChange={(e) => setSearch(e.target.value)} />
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th className="sortable" onClick={() => handleSort('staff_name')}>Name {sortIcon('staff_name')}</th>
                ...
                <tbody>
                  {paginated.length === 0
                    ? <tr><td colSpan={5}><div className="state-box">No staff found.</div></td></tr>
                    : paginated.map((r) => (
                        <tr key={r.id}>
                          <td>
                            <span className="badge badge-green" style={{ cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: 3 }} onClick={() => navigate(`/staff/${r.id}`)}>
                              {r.id}
                            </span>
                          </td>
                          ...
                          <td style={{ textAlign: 'center' }}>
                            <button className="action-btn edit" onClick={() => navigate(`/staff/${r.id}/edit`)}>Edit</button>
