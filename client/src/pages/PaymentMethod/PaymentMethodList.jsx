...
import { getAllPaymentMethods_s, deletePaymentMethod } from '../../api/simpleFormsApi';
...
export default function PaymentMethodList() {
  ...
  const fetchData = async () => {
    ...
    try { const res = await getAllPaymentMethods_s(); setRows(res.data.data || []); }
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(`Delete payment method #${id}?`)) return;
    try { await deletePaymentMethod(id); setRows((prev) => prev.filter((r) => r.id !== id)); }
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      data = data.filter((r) => String(r.id).includes(q) || (r.method_name || '').toLowerCase().includes(q));
    }
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => navigate('/payment-methods/new')}>+ New Method</button>
      </div>

      <div className="card">
        <div className="card-body" style={{ paddingBottom: 0 }}>
          <div className="filter-bar">
            <input className="form-input" style={{ minWidth: 220 }} placeholder="Search ID or method name..." value={search} onChange={(e) => setSearch(e.target.value)} />
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th className="sortable" onClick={() => handleSort('method_name')}>Method Name {sortIcon('method_name')}</th>
                    <th style={{ textAlign: 'center' }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {paginated.length === 0
                    ? <tr><td colSpan={3}><div className="state-box">No payment methods found.</div></td></tr>
                    : paginated.map((r) => (
                        <tr key={r.id}>
                          <td>
                            <span className="badge badge-green" style={{ cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: 3 }} onClick={() => navigate(`/payment-methods/${r.id}`)}>
                              {r.id}
                            </span>
                          </td>
                          <td style={{ fontWeight: 500 }}>{r.method_name}</td>
                          <td style={{ textAlign: 'center' }}>
                            <button className="action-btn edit" onClick={() => navigate(`/payment-methods/${r.id}/edit`)}>Edit</button>
