...
exports.getById = handle(async (req, res) => {
  ...
  if (!d) return res.status(404).json({ success: false, message: 'Staff not found' });
  res.json({ success: true, data: d });
});
exports.create  = handle(async (req, res) => {
  const { staff_name, position, phone } = req.body;
  if (!staff_name) return res.status(400).json({ success: false, message: 'staff_name is required' });
