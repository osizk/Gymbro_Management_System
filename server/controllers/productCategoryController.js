const svc = require('../services/productCategoryService');
...
exports.getById = handle(async (req, res) => {
  ...
  if (!d) return res.status(404).json({ success: false, message: 'Product category not found' });
...
exports.remove  = handle(async (req, res) => {
  ...
  if (!ok) return res.status(404).json({ success: false, message: 'Product category not found' });
